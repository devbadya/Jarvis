...
| `</parameter>`, `</function>`, `</tool_call>` left under the answer        | The opening tag sat inside reasoning that was split off, so the block regex never saw a pair                                                                          | `LEFTOVER_TOOL_TAG` in `src/agent/parse.ts`                                                             |
...
| Skill routed, model did the arithmetic and got it wrong                    | A sum the arithmetic skill claimed is evaluated in code; `groundArithmetic` answers with the calculator's line. A question with no expression still goes to the model | `arithmeticExpression` / `settleArithmetic` in `src/agent/arithmetic.ts`, `groundingFor` in `ground.ts` |
| Model says an appointment is booked and it is not                          | The calendar skill routed but the model never called `calendar`, or it passed a time the parser refused                                                               | `calendar` in `src/tools/calendar.ts`, `resolveWhen` in `src/calendar/when.ts`                          |
| Model says it opened an app and nothing opened                             | The open skill routed but the call never reached the device agent, or no loopback URL is set so the tool is absent                                                    | `openSeed` in `src/agent/device.ts`, `pnpm device`, `configuredDeviceBase` in `src/tools/device.ts`     |
| Follow-up _does he has a wife?_ searched the fragment                      | Pronoun questions start with `does`/`ist`, so they looked new; the query now keeps the last person, including a name that only appeared in chat prose                 | `isPronounFollowUp` / `lastResearchedPerson` in `src/memory/topic.ts`                                   |
| _no of macron has a wife_ skipped research and guessed                     | `isResearchable` required a question mark or an interrogative; informal name-plus-attribute asks now route and are cleaned to `macron wife`                           | `isFactAsk` in `src/skills/researchable.ts`, `factAskQuery` in `ground.ts`                              |
| An English "Thinking Process" shown as the reply                           | Uncapped reasoning spent the whole budget and never closed; it was promoted to the answer. The block is now closed and a second pass answers                          | `reasoningRanOut` in `src/llm/phases.ts`, `generateUncapped` in `src/llm/worker.ts`                     |
| Weather reply copies the exemplar (19.6 °C, 59%) or reads as broken German | The model skipped `weather` or retold the reading badly; the call is seeded and the reply written in code                                                             | `weatherSeed` / `weatherReply` in `src/agent/weather.ts`, `groundingFor` in `ground.ts`                 |
| A clock reply mixes zones or offsets (_20:15 GMT UTC+10_)                  | The model retold the reading; the reply is now written in code from the reading's zone                                                                                | `clockSeed` / `clockReply` in `src/agent/clock.ts`                                                      |
| A research answer is a bare name, or _Die Photosynthese._                  | Extracts are a sentence built from the question; a digest with no extract goes to the model instead of a refusal or the first passage                                 | `researchSentence` / `settleResearch` in `src/agent/ground.ts`                                          |
| A definition answered with a noun, or with invented biology                | The name extractor ran on a definition; now the lead of the article titled exactly the subject is the answer                                                          | `asksDefinition` / `definitionFrom` / `openArticle` in `src/tools/research.ts`                          |
| _Friedrich Merz hat 70 Jahre alt_, or an age from the wrong page           | Age is arithmetic on the birth date in the person's own article, opened by name                                                                                       | `birthOf` / `birthDateIn` in `src/tools/research.ts`, `ageReply` in `src/agent/ground.ts`               |
| A citation pill reads duckduckgo.com                                       | A DuckDuckGo ad arrives as a normal redirect whose target is its own `y.js` link                                                                                      | `resultUrl` in `src/tools/web.ts`, `unwrapDuckDuckGoHref` in `tools/agent-api.ts`                       |
| Advice (_Hast du Tipps?_) answered "no reliable answer found"              | The question stage sent it to `research`; advice now stays with the model                                                                                             | `ADVICE` in `src/skills/researchable.ts`                                                                |
| Greeting echoed back, or a poem preceded by a weather call                 | No skill routed, so every tool was on offer; small talk and creative writing route to skills with `tools: none`                                                       | `src/skills/conversation/`, `src/skills/creative-writing/`                                              |
| A German question answered in English by a skill                           | A capped model copies the nearest exemplar's language; exemplars in the chosen language now go last                                                                   | `nearestInLanguage` in `src/skills/activate.ts`                                                         |
| Small talk answered in a canned sentence, instantly                        | Intended: German and English small talk is written in code; only other languages reach the model                                                                      | `smallTalkReply` in `src/agent/smalltalk.ts`                                                            |
