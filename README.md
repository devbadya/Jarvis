...
**Conversation.** The waveform beside the composer is a spoken turn, not a draft. It listens in the chosen language until you pause, sends that phrase, and reads the reply aloud, then listens again. A second press hangs up. Ending it while a reply is still running stops that reply too. The same browser limit applies: without both speech recognition and speech synthesis there is no button, and dictation steps aside for as long as the conversation is open so the two are not fighting over the microphone.

**Reading aloud.** The speaker on a finished reply reads it with `speechSynthesis`, and a second press stops it. The speaker in the header starts switched on and reads every new reply, in every chat, as it finishes. Switching it off is remembered under `jarvis.speak-replies`. Opening a chat does not read the reply that was already on screen. What is spoken is the prose: the citation line, code fences, bullets and bare URLs are taken off first. The voice is the one for the language you chose: an installed voice when the device has one, and a natural one when that is installed. When the device has no voice for that language, the browser's voice for it is used, and the text of the reply goes to the browser's speech service. The header switch and a live conversation will not both read the same reply.
