...
The interface starts in English. English and German are two pills, first on the landing page above the headline and again in the header, so those two are visible before anyone has read a sentence. Beside them is a menu labelled **Choose language** / **Sprache wählen** listing every other language under the name a speaker of that language looks for — Français, 日本語, العربية, and the rest in `src/i18n/languages.ts`. The choice is kept in `localStorage` under `jarvis.language` and sets `<html lang>`.
