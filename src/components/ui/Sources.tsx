  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      ...
      {labelSources(urls).map(({ url, label }) => (
        <Link
          key={url}
          className="max-w-full rounded-full border border-border/70 bg-surface-secondary/80 px-2.5 py-1 text-xs no-underline transition-colors hover:border-brand/40 hover:bg-surface-hover hover:no-underline"
          href={url}
