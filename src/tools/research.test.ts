...
describe('diverseFirst', () => {
  ...
  it('skips a result with no URL at all', () => {
    expect(diverseFirst([result(''), result('https://news.example/one')], 5)).toHaveLength(1)
  })
})
...
describe('digest', () => {
  ...
  it('numbers each source and puts its URL on the same line as its name', () => {
    expect(digest('Who runs Fictional Airways?', sources)).toBe(
      [
        'Researched "Who runs Fictional Airways?" across 2 sources, all read in full.',
        '',
  ...
  it('caps the whole result, because five whole pages is what this avoids', () => {
...
describe('researchQuestion', () => {
  ...
  it('reports a search that genuinely matched nothing as such', async () => {
    ...
    await expect(researchQuestion(question, config)).resolves.toBe(`No results for "${question}".`)
  })
})
